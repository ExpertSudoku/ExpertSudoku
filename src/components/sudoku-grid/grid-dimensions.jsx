...
function calculateGridDimensions(cellSize, marginSize, fontSize) {
    ...
    // 3x3 blocks are separated by a wider gap instead of bold grid lines,
    // matching the landing page mini grids and the Discord board images.
    const blockGap = 14 * cellSize / 100;
    const cells = [...Array(81).keys()].map((v, index) => {
        ...
        const x = marginSize + (col - 1) * cellSize + Math.floor((col - 1) / 3) * blockGap;
        const y = marginSize + (row - 1) * cellSize + Math.floor((row - 1) / 3) * blockGap;
        return {
            index,
            row,
            col,
    ...
    const dim = {
        ...
        marginSize,
        blockGap,
        // Gap between adjacent cell rects (each cell is inset by half of
        // this); block boundaries additionally get blockGap.
        cellInset: 4 * cellSize / 100,
        cellRadius: 12 * cellSize / 100,
        fontSize: scaledFontSize,
        width: (marginSize * 2) + (cellSize * 9) + (blockGap * 2),
        height: (marginSize * 2) + (cellSize * 9) + (blockGap * 2),
        cell: cells,
        outerPencilOffsets: [
