...
export type BoardParams = {
    title: { number: number; difficulty: string }; // heading: "ExpertSudoku #N — Label" + difficulty icon
...
function renderHeader(title: BoardParams['title']): string {
    ...
    nodes.push(
        ...
        + `${escapeText(`${meta.name} #${title.number}`)}</text>`
    );
    return nodes.join('');
}
...
// Renders a heading (difficulty icon + "ExpertSudoku #N — Label"), up to
